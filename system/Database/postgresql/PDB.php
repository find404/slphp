...
class PDB
{
    ...
    private static function getConnection()
    {
        if (self::$connections == null) {
            ...
            if (!isset($databaseConfig['data']['pgsql'])) {
                exit('数据库配置pgsql数组未设置');
            }

            foreach ($databaseConfig['data']['pgsql'] as $val) {
                self::$connections[] = new \PDO($val['driver'] . ':host=' . $val['host'] . ';port=' . $val['port'] . ';dbname=' . $val['database'], $val['username'], $val['password'], [
